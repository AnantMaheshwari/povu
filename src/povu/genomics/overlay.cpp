...
void pop_exp(const bd::VG &g, const std::vector<pgt::walk_t> &walks,
	     const Overlays &ov, const pt::u32 REF_COUNT,
	     const pgr::raw_variant &pv, pt::u32 wa_idx, pt::u32 wb_idx,
	     std::map<pt::u32, itn_t> &ref_map,
	     std::map<pt::idx_t, std::set<pt::idx_t>> &walk_to_refs,
	     bool &is_tangled, const std::string &rov_id)
{
	bool dbg = rov_id == ">1551>1554" ? true : false;
	dbg = false;
	...
	for (pt::u32 ref_idx{}; ref_idx < REF_COUNT; ref_idx++) {
		...
		for (const auto &[ref_start_idx, ps] : wa_pss) {
			...
			// pgr::var_type_e vt_ = covariant(vt);
			...
			if (dbg && ref_idx == 11) {
				std::cerr
					<< g.get_ref_vec(ref_idx)
						   ->walk->v_ids[ref_start_idx]
					<< "\n";
				std::cerr << " ps: ";
				povu::utils::print_with_comma(std::cerr, ps,
							      ',');
		...
		for (const auto &[ref_start_idx, ps] : wb_pss) {
			...
			// std::cerr << "vt_ " << vt_ << " i " << i << " N " <<
			// N
			//	  << "as _walk_start_idx " << as_walk_start_idx
			//	  << " as_ref_start_idx " << as_ref_start_idx
			//	  << " as_len " << as_len << "\n";
		...
		if (dbg && wa_pss.size() > 1 && wb_pss.size() > 1) {
			std::cerr << "id " << rov_id << " : " << " ref idx "
				  << ref_idx << " wa ps " << a_count
				  << " wb ps " << b_count << "\n";
		}
 ...
 */
void comp_overlays3(const bd::VG &g, const std::vector<pgt::walk_t> &walks,
		    const std::vector<pgr::pairwise_variants> &pv,
		    const pgr::RoV *rov, std::vector<Exp> &rov_exps)
{
	bool dbg = rov->as_str() == ">1551>1554" ? true : false;
	dbg = false;
	...
	Overlays ov = comp_prefixes(g, walks, dbg);
	...
	for (const rov::pairwise_variants &p : pv) {
		...
		for (pt::u32 i{}; i < p.size(); i++) {
			...
			pop_exp(g, walks, ov, g.ref_count(), variants.at(i),
				wa_idx, wb_idx, ref_map, walk_to_refs,
				is_tangled, rov->as_str());

			// if (is_tangled)
			//	std::cerr << "T: " << rov->as_str() << "\n";
...
std::vector<Exp> comp_itineraries3(const bd::VG &g, const pgr::RoV &rov)
{
	...
	comp_overlays3(g, walks, pv, rov_, rov_exps);
	return rov_exps;
}
