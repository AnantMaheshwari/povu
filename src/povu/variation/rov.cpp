...
std::optional<genomic_region> parse_genomic_region(const std::string &region_str)
{
	...
	if (colon_pos == std::string::npos) {
		ERR("Invalid region format '{}': missing colon separator",
		    region_str);
		return std::nullopt;
	}
	...
	if (dash_pos == std::string::npos) {
		ERR("Invalid region format '{}': missing dash separator in range",
		    region_str);
		return std::nullopt;
	}
	...
	for (char c : start_str) {
		if (!std::isdigit(c)) {
			ERR("Invalid region format '{}': non-numeric start position",
			    region_str);
			return std::nullopt;
		}
	}
	for (char c : end_str) {
		if (!std::isdigit(c)) {
			ERR("Invalid region format '{}': non-numeric end position",
			    region_str);
			return std::nullopt;
		}
	}
	...
	try {
		...
		if (start >= end) {
			ERR("Invalid region '{}': start position ({}) must be less than end position ({})",
 ...
 * A vertex overlaps if any of its constituent graph vertices have positions
 * within the region on the specified reference path
 */
bool pvst_vertex_overlaps_region(const bd::VG &g, const pvst::VertexBase *pvst_v,
				 const genomic_region &region, pt::id_t ref_id)
{
	// Get the route parameters which contain source and sink vertex IDs
	auto opt_route = pvst_v->get_route_params();
	if (!opt_route.has_value()) {
		return false; // vertices without route params are skipped
	}
		    ...
		    const std::optional<pt::id_t> &region_ref_id)
{
	...
	for (pt::u32 i : colored_vtxs) { // i is pvst_v_idx
		...
		if (region.has_value() && region_ref_id.has_value()) {
			if (!pvst_vertex_overlaps_region(g, v, region.value(),
							 region_ref_id.value())) {
				continue; // Skip vertices outside the region
			}
		}
	...
	const std::optional<genomic_region> &region)
{
	...
	if (region.has_value()) {
		...
		if (!region_ref_id.has_value()) {
			...
			WARN("No RoVs will be generated due to invalid region");
			return rs; // Return empty result
		}
		INFO("Filtering RoVs to region {}:{}-{}", region.value().ref_name,
	...
	for (pt::idx_t i{}; i < pvsts.size(); i++) { // for each pvst
		...
		find_pvst_rovs(g, pvst, colored_vtxs, rs, region, region_ref_id);
		// reset progress bar
		// ProgressBar bar;
